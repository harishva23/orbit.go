...
func ElasticConsume(ctx context.Context, js jetstream.JetStream, streamName string, consumerGroupName string, memberName string, messageHandler func(msg jetstream.Msg), config jetstream.ConsumerConfig, pullMaxMessages ...int) (ConsumerGroupConsumeContext, error) {
	var err error
	...
	if instance.Config.IsInMembership(memberName) {
		instance.joinMemberConsumer(pullMaxMessages...)
	}
...
func CreateElastic(ctx context.Context, js jetstream.JetStream, streamName string, consumerGroupName string, maxNumMembers uint, partitioningFilters []PartitioningFilter, maxBufferedMessages int64, maxBufferedBytes int64) (*ElasticConsumerGroupConfig, error) {
	...
	if len(consumerGroupConfig.PartitioningFilters) > 0 {
		...
		subjectTransforms = append(subjectTransforms, jetstream.SubjectTransformConfig{Source: ">",
			Destination: getPartitioningTransformDest(PartitioningFilter{Filter: ">", PartitioningWildcards: []int{}}, config.MaxMembers),
		})
	}
...
func ElasticGetPartitionFilters(config ElasticConsumerGroupConfig, memberName string) []string {
	...
	if len(config.PartitioningFilters) > 0 {
		...
		filters = GeneratePartitionFilters(config.Members, config.MaxMembers, config.MemberMappings, memberName, ">")
	}
...
func (instance *ElasticConsumerGroupConsumerInstance) joinMemberConsumer(pullMaxMessages ...int) {
	var err error
	...
	instance.startConsuming(pullMaxMessages...)
}
...
func (instance *ElasticConsumerGroupConsumerInstance) startConsuming(pullMaxMessages ...int) {
	var err error
	instance.consumerConsumeContext, err = instance.consumer.Consume(instance.consumerCallback, jetstream.PullExpiry(max(instance.consumerUserConfig.AckWait/pullTimeoutDivider, minPullExpiryPinnedTTL)), jetstream.PullPriorityGroup(priorityGroupName), jetstream.PullMaxMessages(pullMaxMessages[0]))
	if err != nil {
...
func validateConfig(config ElasticConsumerGroupConfig) error {
	...
	for _, pf := range config.PartitioningFilters {
		...
		if len(pf.PartitioningWildcards) > numWildcards {
			return errors.New("the number of partitioning wildcards must not be larger than the total number of * wildcards in the filter")
		}
...
func getPartitioningTransformDest(partitioningFilter PartitioningFilter, maxMembers uint) string {
	...
	if len(partitioningFilter.PartitioningWildcards) == 0 {
		return fmt.Sprintf("{{Partition(%d)}}.%s", maxMembers, destFromFilter)
	}
